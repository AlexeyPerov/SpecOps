...
import { mappedSessionForAgent } from "./workspaceAgentSession";
 ...
 * `session.messages` payload as the display source of truth for each agent tab
 * that has a linked OpenCode session. Local snapshot remains as offline cache /
 * fallback (per Q3 decision). Failures are non-fatal — the local snapshot stays
 * in place if hydration can't reach the server.
 */
export async function hydrateWorkspaceAgentMessages(input: {
  ...
  await Promise.all(
    agents.map(async (entry) => {
      const mapping = mappedSessionForAgent(agents, entry.id);
      if (!mapping) {
        return;
      }
      try {
