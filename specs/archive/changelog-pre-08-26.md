...
## 2026-08-11 21:54 MSK — Replace AI roadmap with unified multi-runtime Sessions

Reframed SpecOps AI as one workspace-scoped **Sessions** control plane rather
than separate Chat, Cloud, and OpenCode lanes. The new source of truth is
`specs/ops/roadmap.md` plus the Agent Sessions decisions and execution plan.

- Runtime is immutable per session. Initial delivery order is Claude → Codex →
  OpenCode → Cursor; ZCode is out of scope.
- Standalone HTTP Chat is scheduled for removal. The planned Cursor Cloud
  activity-rail context is cancelled; a future cloud option may only be a
  runtime-specific session target.
- Introduced the bundled **Agent Host** direction: a Tauri-supervised TypeScript
  sidecar owns vendor SDKs/processes, credentials, native session ids, health,
  and normalized events behind a versioned stdio JSON-RPC protocol.
- Replaced the large OpenCode-shaped backend target with a small mandatory
  session core plus capability-driven runtime extensions.
- Locked shared-workspace semantics: every local agent uses the real workspace
  `cwd`; SpecOps adds warnings and observability but no worktrees, writer locks,
  automatic git operations, serialization, or rollback.
- Defined cross-runtime handoff as a new target-native session seeded with a
  user-reviewable context packet; native histories are never merged.
- Added milestone plan A0–A5 covering Chat removal/foundation, Claude, Codex,
  OpenCode host migration, Cursor, handoff/concurrency observability, packaging,
  validation, and secret redaction.
- Classified former phases 4–7 as cancelled/superseded in the new roadmap and
  marked the tracked phase-2/phase-3 documents as legacy/historical baselines.
