...
describe("opencodeSettings", () => {
  it("defaults to sidecar settings with enabled false (beta opt-in)", () => {
    expect(normalizeOpencodeSettings(undefined)).toEqual(defaultOpencodeSettings);
    expect(defaultOpencodeSettings.enabled).toBe(false);
  ...
  it("normalizes enabled from input", () => {
    ...
    expect(normalizeOpencodeSettings({ mode: "sidecar" }).enabled).toBe(false);
    expect(normalizeOpencodeSettings({ enabled: true, mode: "sidecar" }).enabled).toBe(true);
  });
  ...
  describe("isOpencodeEnabled", () => {
    it("returns false for default settings (beta opt-in)", () => {
      expect(isOpencodeEnabled(defaultOpencodeSettings)).toBe(false);
    });

    it("returns true when enabled is true", () => {
      expect(isOpencodeEnabled({ ...defaultOpencodeSettings, enabled: true })).toBe(true);
    });

    it("returns false for null/undefined input", () => {
      expect(isOpencodeEnabled(null)).toBe(false);
      expect(isOpencodeEnabled(undefined)).toBe(false);
    });
  });
});